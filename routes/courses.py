...
@course_router.get("/students/{course_id}", response_model=course_schema.CourseStudents)
def get_students_by_course(course_id: int, db: Session = Depends(get_db)):
    return crud.get_students_by_course(db, course_id)


@course_router.put("/students/{course_id}", response_model=course_schema.CourseStudents)
def update_students_by_course(course_id: int, db: Session = Depends(get_db), file: UploadFile = File(...)):
    crud.delete_students_by_course(db, course_id)
    # Obtenemos el curso de la base de datos
    db_course = crud.get_course(db=db, course_id=course_id)
    # Obtenemos el nivel y paralelo del curso
    id = db_course.id
