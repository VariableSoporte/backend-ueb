...
@list_document_router.post("/{list_id}/{document}", response_model=list_document_schema.ListDocument)
def add_document(file: UploadFile = File(...), list_id: int = 0, db: Session = Depends(get_db), document: str = "document"):
    try:
        ...
        # Crear la ruta donde se almacenará el documento
        ...
        with open(file_path, "wb") as document_file:
            document_file.write(file.file.read())
...
def update_list_document(document_type: str, file: UploadFile = File(...), document_id: int = 0, db: Session = Depends(get_db)):
    try:
        ...
        file_path = os.path.join(
            f"docs/lists/{document.list_id}", file.filename)
        # Guardar el documento en la ruta especificada
        with open(file_path, "wb") as document_file:
            document_file.write(file.file.read())
