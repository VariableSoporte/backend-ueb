...
@list_router.post("/image/{list_id}")
def upload_photo(file: UploadFile = File(...), list_id: int = 0, db: Session = Depends(get_db)):
    ...
    db_list = crud.update_list(
        db=db, list_id=list_id, file=file_path)
    return db_list


@list_router.get("/image/{list_id}")
async def get_image(list_id: str, db: Session = Depends(get_db)):
    # Directorio de imágenes"
